...
* **`lakefile.toml`**: The Lake package manifest. We deliberately keep this minimal:
  ...
  * Various `lean_exe` targets (`app1`, `app2`, `verso_site`) representing artifacts to be built (binaries, static sites).
...
* **`Verso/`**: Contains code for Verso books, slides, or blogs. The main entry point is `Verso.Main`, which gets compiled into the `verso_site` executable artifact.
